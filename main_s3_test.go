...
func TestStartS3ServerRejectsVirtualHostedStyleConfig(t *testing.T) {
	...
	srv, err := startS3Server(ctx, &net.ListenConfig{}, eg, "localhost:9000", nil, nil, nil, "us-east-1", "", false, nil, nil)
...
func TestStartS3ServerAllowsEmptyAddress(t *testing.T) {
	eg, ctx := errgroup.WithContext(context.Background())
	srv, err := startS3Server(ctx, &net.ListenConfig{}, eg, "", nil, nil, nil, "us-east-1", "", false, nil, nil)
