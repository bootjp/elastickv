...
# doc (docs/design/2026_06_02_implemented_composed1_m5_jepsen_route_shuffle.md
...
SPLIT_BIN=/tmp/elastickv4-m5-split
...
if ! $NO_REBUILD; then
  ...
  # in a machine-readable way (review feedback on PR #924).
  ...
  echo "[build] compiling elastickv-split helper..."
  # Used by the M5b route-shuffle nemesis to issue SplitRange.
  go build -o "$SPLIT_BIN" ./cmd/elastickv-split
  echo "[build] done -> $BINARY, $ROUTE_KEY_BIN, $LIST_ROUTES_BIN, $SPLIT_BIN"
fi
...
# encodeDynamoSegment (review feedback on PR #905 ffb9c73f).
#
...
# script (review feedback on PR #924).
for bin in "$ROUTE_KEY_BIN" "$LIST_ROUTES_BIN" "$SPLIT_BIN" "$BINARY"; do
  if [ ! -x "$bin" ]; then
...
if ! $NO_CLUSTER; then
  ...
  # the Raft / Dynamo ports for the next run (review feedback on
  # PR #924).
  ...
  #                      MUST NOT appear here (review feedback on
  #                      PR #905 3ca2a7f7).
...
# is the canonical one — see review feedback on PR #924.
