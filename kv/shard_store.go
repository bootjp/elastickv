...
var ErrRouteWriteBelowFloor = ErrRouteWriteTimestampTooLow
...
func (s *ShardStore) ScanGroupAt(ctx context.Context, groupID uint64, start []byte, end []byte, limit int, ts uint64) ([]*store.KVPair, error) {
	...
	routeStart, routeEnd := readinessRouteRangeForScan(start, end)
	if err := s.verifyExplicitGroupRoutesForRange(ctx, groupID, routes, routeStart, routeEnd); err != nil {
		return nil, err
	}
	return s.scanExplicitGroupRoutesAt(ctx, routes, start, end, limit, ts, clampToRoutes)
}
...
func (s *ShardStore) scanRouteAtDirectionWithReadFenceRouteFilterPage(
	...
	if isLinearizableRaftLeader(ctx, engineForGroup(g)) {
		return s.scanRouteAtLeaderRouteFilter(ctx, g, route, start, end, limit, visibleLimit, ts, reverse, routeStart, routeEnd)
	}
...
func (s *ShardStore) scanRouteAtDirectionPhysicalLimit(
	...
	if isLinearizableRaftLeader(ctx, engineForGroup(g)) {
		return s.scanReadyLeaderPhysicalLimit(ctx, g, route, start, end, visibleLimit, physicalLimit, ts, reverse)
	}
