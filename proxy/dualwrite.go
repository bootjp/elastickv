...
const (
	...
	// limiter, so the command caps the derived value by the remaining backend
	// connection budget.
...
func (d *DualWriter) Blocking(ctx context.Context, cmd string, args [][]byte) (any, error) {
	...
	if d.hasSecondaryWrite() {
		...
		} else if shouldReplayBlockingToSecondary(cmd) && blockingResultMayHaveMutated(resp, err) {
			d.goBlockingReplay(func(ctx context.Context) {
