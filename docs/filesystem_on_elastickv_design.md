...
1. Stable inode number (`st_ino`) for object lifetime; root inode is fixed.
2. `getattr` returns coherent `mode`, `nlink`, `uid`, `gid`, `size`, `mtime`, `ctime`, `atime` with nanosecond precision.
3. `readdir` supports resumable offset cookies and always emits `"."` and `".."`.
...
1. `inode_id`: 64-bit monotonically allocated ID (`root=1`) to map directly to FUSE `st_ino`.
2. `home_slot`: 64-bit placement token derived at create time.
3. `chunk_index`: 64-bit unsigned index.
...
Extend shard route normalization (currently `kv/routeKey` behavior for internal key families) to treat:
...
2. Offset cookie is an opaque cursor derived from the last emitted name.
3. Response always includes `"."` and `".."` first, then normal entries.
...
Each intent stores step cursor for resume after crash.
...
10. `Create(parent_inode, name, mode, uid, gid) -> inode, fh_id`
...
1. Return typed filesystem errors (`ENOENT`, `EEXIST`, `ENOTDIR`, `EISDIR`, `ENOTEMPTY`, `EXDEV`, `EPERM`, `EACCES`, `EINVAL`, `EFBIG`, `ENOSYS`).
...
|---|---|---|
| `lookup` | `Resolve` + `GetAttr` | Stable inode and generation |
...
| `readdir` | `Readdir` | Stable cookie pagination, includes dot entries |
