...
- All endpoints use plaintext transport (no TLS) and are unauthenticated by default: gRPC (including `RaftAdmin`), Redis API, DynamoDB-compatible API, and S3-compatible API. The S3-compatible API remains plaintext HTTP even when `--s3CredentialsFile` is configured; that option only enables SigV4 authentication.
