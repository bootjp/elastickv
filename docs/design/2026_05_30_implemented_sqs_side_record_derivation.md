...
For every `!sqs|msg|data|...` record M5-1 writes, the encoder emits the **derived** side records the live adapter would have written:
...
| -------------- | ------------------------------------------------------------------------ | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `vis`          | `!sqs\|msg\|vis\|<queue-seg><gen-BE><visibleAt-BE><msgID-seg>`           | `[]byte(messageID)`       | One row per data record. **`visibleAt` is restored to `available_at_millis`**. Already-available messages are visible immediately; delayed messages remain invisible until their scheduled time. |
...
**Source-file attribution** (M5-2 is classic-only — `*Dispatch` wrappers in `adapter/sqs_keys_dispatch.go` route between classic and partitioned constructors and are NOT needed here; M5-3 implements the partitioned constructors separately):
...
## Implementation files (M5-2 slice)
...
Wire into `encode_sqs.go`'s existing `encodeQueueMessages` walk: after `addMessage(...)` emits `!sqs|msg|data|`, call `addSideRecords(...)` to emit the matching `vis` + `byage` (always) and `dedup` (FIFO + resolved non-empty dedup ID only) rows in the same loop. **No `group` rows ever.** No new disk reads.
...
| `TestSQSEncodeSideRecordsEmptyDedupOmitsDedupRow`                 | FIFO message with empty `message_dedup_id` and no content-based dedup: data + vis + byage emitted; dedup row **NOT** emitted.                                                           |
| `TestSQSEncodeSideRecordsCBDDerivesDedupID`                       | Content-based-dedup FIFO message with empty `message_dedup_id` emits a dedup row keyed by `sha256(body)`, matching live send behavior.                                                   |
| `TestSQSEncodeSideRecordsNoGroupRows`                             | FIFO queue with N messages across M distinct groups: **zero** `!sqs\|msg\|group\|` rows emitted (any row falsely indicates a held lock and blocks every group post-restore). |

`TestSQSEncodeSideRecordsCrossCheckClassic` is the §"Encoder cross-check" pattern the parent doc §"Per-adapter reverse encoders" mandates. Cluster-boot ReceiveMessage and reaper end-to-end tests are not shipped in this slice; the implemented evidence is encoder-level byte-shape and decode round-trip coverage.
...
4. **Data consistency.** Group rows MUST NOT be emitted at all: `loadFifoGroupLock` treats key presence as "lock held" with no graceful "empty owner = no lock" decode path, so any row would permanently block receives for the group (pinned by `TestSQSEncodeSideRecordsNoGroupRows`). Dedup rows MUST be gated on a resolved non-empty dedup ID: explicit `message_dedup_id`, or SHA-256(body) for content-based-dedup FIFO queues. FIFO-only families (dedup) MUST be gated on `meta.FIFO`; emitting them for standard queues poisons the keyspace. All three rules pinned by tests above.
