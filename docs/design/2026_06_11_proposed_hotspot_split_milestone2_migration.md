...
| M2-PR6 | Cross-group end-to-end: ExportRangeVersions / ImportVersions server-side handlers + migrator BACKFILL/DELTA_COPY + raw-candidate staged/live merge read path in both scan directions + §7.2.2e source-side cutover read-fence arm with every-source-voter ACK, catalog-version waiter, route-key-normalized scan ownership checks, and server-stamped RawKV read versions | integration incl. delete/TTL DELTA_COPY, Redis list/hash/set/zset/stream migration, HLC restart/fence-floor cases + `kv/fsm_cutover_read_fence_test.go` |
| M2-PR7 | Target-local `PromotionState` + background promoter + ordered default-group promotion-complete CAS retaining `min_write_ts_exclusive` + source/target cleanup before DONE history move; AbandonSplitJob + CLEANUP GC + Jepsen split workload | `kv/fsm_promote_staged_test.go` raw hidden-version merge + jepsen suite |
...
7. **Same-group observability.** Resolved for M2: same-group splits stay on the M1 `SplitRange` fast path and do not create `SplitJob` records. If operators need same-group split audit records later, add a separate audit event rather than sending `target_group_id == source_group_id` through `StartSplitMigration`.
