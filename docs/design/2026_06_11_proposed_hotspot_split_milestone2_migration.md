...
| `snapshot_min_prepared_ts` | uint64 | **Running** minimum `commit_ts` of any prepare *ever observed in-flight* for the moving range, sampled by §3.2a.0a's route-faithful lock scan (`!txn|lock|` family scan filtered by `routeKey(lockedKey) ∈ [routeStart, routeEnd)`, not a raw `txnLockKey` bracket) **on every migrator tick from BACKFILL through §3.2a step 0b's post-fence drain** (§6.1.0 step 2). Monotone-down; persisted on each advance. Final once `post_fence_drain_completed = true` — no new prepare can land after the fence, so no future commit can lower it. `0` means no prepare was ever observed. |
...
#### 6.1.0 `snapshot_ts` selection and DELTA_COPY lower bound — must dominate every prepare that is in-flight at *any* point before the fence (closes codex round-14 P1 line 405; closes codex round-15 P1 — fence-before-boundary)
...
2. **Track the running minimum until the fence blocks new prepares.** From BACKFILL through §3.2a step 0b's post-fence drain, the migrator maintains `snapshot_min_prepared_ts` = the smallest `commit_ts` of any txn whose primary resolves into the moving range and that prepared after the provisional scan (monotone-down, persisted on each advance). Because the source is still `Active` during BACKFILL and FENCE-apply, new prepares with arbitrarily low caller-supplied / lagging-clock `commit_ts` can keep appearing; the running minimum must capture every one. To avoid an inter-tick gap — a prepare that both lands and resolves between two ticks would never appear in a *live-lock* sample — the running minimum is taken over **both** durable txn families, route-faithful by `routeKey()`:
   - the live `!txn|lock|` family (in-flight prepares), and
   - the durable `!txn|cmt|` commit-record family (`txnCommitKey`, `kv/txn_keys.go:50`), keyed `!txn|cmt|<primaryKey>|<startTS>`, which persists after the intent lock is deleted until GC/compaction (`kv/compactor.go`). A txn that prepared and committed entirely within one inter-tick interval leaves a commit record, so its `commit_ts` is still observed at the next tick even though its lock is gone.

   The minimum is only **final** once §3.2a step 1's unanimous voter-ACK has made every source voter reject new prepares (`verifyRouteNotFenced`) **and** step 0b's post-fence drain has returned empty — at that point no future prepare can land in the moving range, and every txn that committed in the pre-fence window left a durable commit record the final scan will read, so no future commit can lower the minimum. (The commit-record retention horizon comfortably exceeds a migration's pre-fence window — BACKFILL + FENCE-apply is bounded by a few txn-TTLs; if a deployment compacts commit records aggressively, the M2-PR0 lock-`commit_ts` wire change keeps the record live until its job's `delta_floor` is finalised.)
3. **Finalise the boundary after the post-fence drain.** When step 0b's drain returns empty (`post_fence_drain_completed = true`), the route-faithful lock+commit-record scan is run **one last time**; the lock side returns empty by construction (no in-flight prepare survives the drain) and the commit-record side yields the final running minimum. The DELTA_COPY lower bound is then fixed as `delta_floor = min(snapshot_ts, snapshot_min_prepared_ts - 1)` and DELTA_COPY exports the window **`(delta_floor, fence_ts]`** (not `(snapshot_ts, fence_ts]`). Any post-scan prepare that committed with `commit_ts ≤ snapshot_ts` was observed in the step-2 running minimum (via its lock or its commit record), so `delta_floor ≤ snapshot_min_prepared_ts - 1 < commit_ts`, i.e. `commit_ts > delta_floor` and the version falls **inside** DELTA_COPY's window — it is no longer dropped. `delta_floor` is persisted on the SplitJob alongside `snapshot_min_prepared_ts` so a DELTA_COPY resume uses the same lower bound.
