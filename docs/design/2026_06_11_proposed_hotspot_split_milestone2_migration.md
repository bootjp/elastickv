...
| `snapshot_ts` | uint64 | HLC commit-ts pinned at BACKFILL entry. Selected by §6.1.0: `min(HLC.Next(), snapshot_min_prepared_ts - 1)` so every commit with `commit_ts ≤ snapshot_ts` has its prepare resolved at scan time (closes codex round-14 P1 line 405). |
| `snapshot_min_prepared_ts` | uint64 | Smallest `commit_ts` among in-flight prepares for the moving range at BACKFILL-entry scan time, found via §3.2a.0a's route-faithful lock scan (`!txn|lock|` family scan filtered by `routeKey(lockedKey) ∈ [routeStart, routeEnd)`, not a raw `txnLockKey` bracket). Persisted so a migrator restart sees the same `snapshot_ts` derivation. `0` means no prepare existed. |
...
#### 6.1.0 `snapshot_ts` selection — must dominate every in-flight prepare for the moved range (closes codex round-14 P1 line 405)

The corner case: 2PC's `commit_ts` is allocated by the coordinator at **prepare time** (via `HLC.Next()` on the prepare path), not at commit-apply time. Concretely, a transaction `T` can sequence as:

1. `t = t_prepare`: coordinator picks `commit_ts(T) = HLC.Next()` and writes intent locks on the participant groups.
2. `t = t_snapshot`: migrator picks `snapshot_ts = HLC.Next()` for BACKFILL, with `t_prepare < t_snapshot` so `commit_ts(T) < snapshot_ts` is possible (depending on coordinator vs migrator interleaving of `HLC.Next()` calls).
...
If the migrator naively starts BACKFILL at `snapshot_ts` chosen by `HLC.Next()` and the export reads the snapshot at exactly `maxCommitTS = snapshot_ts`, the committed value lands at `commit_ts(T) ≤ snapshot_ts` but is **not yet present in the store** when BACKFILL iterates. DELTA_COPY's `(snapshot_ts, fence_ts]` window then excludes it (because `commit_ts(T) ≤ snapshot_ts`), and the FENCE drain doesn't help because the drain only waits for intents to disappear, not for past-committed-but-not-yet-applied versions to surface. Net result: a committed write the client has already observed is silently dropped from the target. This is a fresh hazard — codex round-14 P1 line 405 spotted it.

**The fix is to pick `snapshot_ts` such that no prepare with `commit_ts ≤ snapshot_ts` is still in-flight.** Concretely, the migrator does the following before opening BACKFILL:

1. **Scan in-flight intents.** Run the **route-faithful lock scan** of §3.2a.0a — the `!txn|lock|` family bracket `[txnLockPrefix(), txnLockPrefixEnd())` filtered by `routeKey(lockedKey) ∈ [routeStart, routeEnd)`, **not** a raw `[txnLockKey(routeStart), txnLockKey(routeEnd))` bracket (which would miss DynamoDB/SQS/Redis/list/S3 item locks whose raw storage key sorts outside the routing-key bounds — see §3.2a.0a). For each accepted lock, decode its recorded `commit_ts` (the lock value already carries it for the existing lock-resolver; if it does not in the current implementation, the M2-PR0 wire change adds it — same Raft-applied lock-write path). Take `min_prepared_commit_ts = min(commit_ts for all in-flight prepares accepted by the filter)`. If no prepare exists in the moving range, set `min_prepared_commit_ts = ∞`.
2. **Pick `snapshot_ts`.** `snapshot_ts = min(HLC.Next(), min_prepared_commit_ts - 1)`. This guarantees every commit with `commit_ts ≤ snapshot_ts` is **already applied** (its prepare has resolved before the scan — otherwise it would have been in the scan result), so the BACKFILL iterator sees it. Every commit with `commit_ts > snapshot_ts` falls into the DELTA_COPY `(snapshot_ts, fence_ts]` window (since `fence_ts ≥ snapshot_ts` and DELTA_COPY runs until FENCE).
3. **Persist in SplitJob.** `snapshot_ts` and the scanned `min_prepared_commit_ts` go into the SplitJob row (`snapshot_ts`, `snapshot_min_prepared_ts`) so a migrator restart between this step and BACKFILL completion knows the same `snapshot_ts`.

The alternative — "wait for all currently-prepared transactions to resolve, then `snapshot_ts = HLC.Next()`" — is also correct but requires waiting for arbitrary txn TTLs before BACKFILL can even start. Picking `snapshot_ts = min_prepared_commit_ts - 1` is cheaper: it pushes the boundary *down* below the smallest in-flight prepare instead of waiting them out, and DELTA_COPY catches the commits that land between `snapshot_ts` and `fence_ts` regardless.

Edge case: if `min_prepared_commit_ts ≤ source_local_LastCommitTS`, the migrator's snapshot might exclude very recent committed history. That is acceptable — DELTA_COPY's `(snapshot_ts, fence_ts]` is a proper superset of the missing window and exports the same versions; the only cost is that DELTA_COPY has slightly more work to do. The HLC ceiling advancement in §6.2.1 still uses `max_imported_ts` (which includes DELTA_COPY's exported versions) so the target-side time-travel invariant is unchanged.
