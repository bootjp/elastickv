...
| `snapshot_min_prepared_ts` | uint64 | Smallest `commit_ts` among in-flight prepares for `[routeStart, routeEnd)` at BACKFILL-entry scan time. Persisted so a migrator restart sees the same `snapshot_ts` derivation. `0` means no prepare existed. |
...
1. **Scan in-flight intents.** Scan `!txn|lock|<encoded>[routeStart, routeEnd)` and decode each lock's recorded `commit_ts` (the lock value already carries it for the existing lock-resolver; if it does not in the current implementation, the M2-PR0 wire change adds it — same Raft-applied lock-write path). Take `min_prepared_commit_ts = min(commit_ts for all in-flight prepares)`. If no prepare exists, set `min_prepared_commit_ts = ∞`.
