...
| `source_retention_pin_ts` | uint64 | Source-group MVCC retention floor installed before BACKFILL opens (§6.1.0a). Initially `snapshot_ts`; after the tracker is armed, every source-group apply that records a lower migration-window `commit_ts` also lowers the active pin to `min(current_pin, commit_ts - 1)` in the same Raft entry, and the default-group SplitJob records the monotone-down witness on the next migrator tick. Finalising `delta_floor` may lower it again to `min(source_retention_pin_ts, delta_floor)`, but compaction safety does not wait for that finalisation. Released only after successful source cleanup or the abandon/FAILED cleanup path disarms the job. |
...
2. **Track the running minimum until the fence blocks new writes — over every *key or range footprint* in the moving range, not only moved primaries (closes codex round-16 P1; closes codex round-18 P1 on historical-version over-scan; closes codex round-20 P1 on raw/one-phase low-ts writes; closes codex round-21 P1 on `DEL_PREFIX` range writes).** From BACKFILL through §3.2a step 0b's post-fence drain, the migrator maintains `snapshot_min_admitted_ts` = the smallest `commit_ts` in the source-group migration write tracker plus any still-live pre-tracker locks whose locked key resolves into the moving range (monotone-down, persisted on each advance). The same source-group Raft apply that appends a tracker row with `commit_ts < source_retention_pin_ts` also lowers the active source retention pin to `commit_ts - 1` before compaction can observe the old floor; the SplitJob's `source_retention_pin_ts` witness is updated by the migrator after observing that source-side state. The earlier wording — "scan committed MVCC versions in the moving range with `commit_ts ≤ snapshot_ts`" — was too broad: it includes every old retained version that BACKFILL already copied, so a years-old historical version could lower `delta_floor` near zero and turn DELTA_COPY into another full backfill. The tracker is the correctness backstop for same-tick land-and-resolve prepares, for raw point / one-phase writes that create no intent lock, and for `DEL_PREFIX` range tombstones whose prefix intersects the moving route; ordinary committed MVCC history is copied by BACKFILL/DELTA_COPY but is **not** allowed to lower the boundary.
   ...
   M2-PR0 also adds a route-local **per-key transaction-success marker** for committed participants: `!txn|ok|<locked_key>|<start_ts>|<commit_ts>|<primary_key>` (exact byte encoding versioned with the txn key codecs). The commit apply path writes this marker in the same source/target Raft apply that installs the committed MVCC version for each participant key; its value redundantly encodes `(locked_key, primary_key, start_ts, commit_ts)` and a small marker version. Because `locked_key` is the first routed component, `routeKey(txnSuccessKey)` equals `routeKey(locked_key)`, so a secondary key that moves without its primary carries local identity-bearing proof through BACKFILL/DELTA_COPY. The export bracket planner treats `!txn|ok|` as a concrete txn family bracket, the `routeKey()` decoder understands it, and cleanup/txn-GC removes it with the same retention horizon as the primary commit/rollback markers after no idempotent retry can need it. Post-drain COMMIT idempotency may return success from this marker (or a primary commit record / identity-bearing commit marker); it must not fall back to an ordinary committed MVCC version that lacks `(primary_key,start_ts)`.
...
Before BACKFILL opens, the source already has the conservative route-scoped pin installed by `ArmMigrationWriteTracker` in §6.1.0 step 1. After `snapshot_ts` is chosen and the initial route-faithful lock scan / current tracker rows have produced `snapshot_min_admitted_ts`, the migrator may replace the conservative no-prune floor with the current safe floor `min(snapshot_ts, snapshot_min_admitted_ts - 1)` and records that value as `source_retention_pin_ts` in the SplitJob. The pin is route-scoped and bracket-aware: compaction may still discard unrelated keys, but for any raw row accepted by the §6.3.1 `RouteKeyFilter` it must retain committed versions and tombstones needed to answer `ExportVersions` at the current BACKFILL/DELTA_COPY lower bound. There is no window where tracked writes are admitted while the moving route has no source-local retention pin.

After the tracker is armed, the retention floor is not allowed to wait for final `delta_floor` computation. Whenever the source FSM records a migration-window write with `commit_ts < current_pin`, it atomically lowers the active pin to `commit_ts - 1` in the same source-group Raft apply that appends the `!migwrite|<job_id>|*` row. This closes the compaction race where a low-ts write lands during BACKFILL/FENCE, `snapshot_min_admitted_ts` remembers it, but compaction prunes the older version/tombstone before DELTA_COPY finalises the lower bound. For pre-tracker locks discovered by the initial route-faithful lock scan, the migrator lowers the pin before choosing `snapshot_ts`; for later tracker rows, the source apply path owns the immediate lower. The default-group SplitJob records the monotone-down `source_retention_pin_ts` as an audit/restart witness, but the source-local pin is the safety mechanism.

When §3.2a finalises `delta_floor`, the migrator updates the same source-group pin to `min_ts = min(source_retention_pin_ts, delta_floor)` before DELTA_COPY starts if that value is lower than the already-applied running pin. This is a reconciliation step, not the first moment compaction becomes safe. The full `(delta_floor, fence_ts]` window remains readable even when a low caller-supplied write pushed `delta_floor` below the provisional snapshot.
...
- `kv/fsm_current_owner_gate_test.go`: post-CUTOVER owner gate for writes. After the source route is removed and `!migfence` is cleared, point writes, one-phase writes, prepares, and `DEL_PREFIX` ranges whose normalized footprint intersects the moved right child must return `ErrRouteMoved` / `ErrRouteOwnershipUnknown` on the old source even with `ObservedRouteVersion=0`. Red controls: relying only on `verifyRouteNotFenced` accepts the write because no `WriteFenced` route remains; relying only on `verifyComposed1` accepts zero-version requests.
...
| M2-PR3 | `store/MVCCStore.ExportVersions` raw committed-version export + `ImportVersions` with idempotency, tombstone/`expire_at` preservation, target-local full-HLC floor persistence and descriptor `min_write_ts_exclusive` plumbing, no migrator wiring | store unit incl. tombstone/TTL + HLC-floor restore cases + property |
| M2-PR4 | `distribution/migrator.go` state machine + export bracket planner, including list delta/claim, Redis wide-column brackets, concrete-only SQS/S3 brackets, and matching `routeKey()` decoder coverage — same-group target (no-op data move) to lock the state shape | migrator unit + `migrator_export_plan_test` |
