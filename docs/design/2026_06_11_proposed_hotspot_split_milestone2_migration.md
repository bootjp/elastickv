...
| Data consistency | Composed-1 alignment at CUTOVER; reader at `≤ fence_ts` on source vs reader at `≥ catalog_version+1` on target; target-local full-HLC floor across restart/snapshot; raw staged/live merge suppresses hidden tombstone/TTL winners; internal-key route mapping. |
...
| M2-PR6 | Cross-group end-to-end: ExportRangeVersions / ImportVersions server-side handlers + migrator BACKFILL/DELTA_COPY + raw-candidate staged/live merge read path + §7.2.2e source-side cutover read-fence arm | integration incl. delete/TTL DELTA_COPY + HLC restart cases + `kv/fsm_cutover_read_fence_test.go` |
