...
**Routing requires two ordered checks — empty-segment rejection BEFORE `path.Clean`.** The original `r.URL.Path` is split on `/` and inspected for any **empty segment** between `queues` and the next non-empty token. If found, the handler returns `400 Validation` (`"queue name is required"`) before any further dispatch. Only AFTER that check does the handler apply `path.Clean` to normalise leading / trailing / duplicated slashes and `.` / `..` traversal. The order matters: `path.Clean` is destructive — `"/admin/api/v1/sqs/queues//messages"` Cleans to `"/admin/api/v1/sqs/queues/messages"`, which would otherwise be interpreted as the messages sub-resource of a queue named literally `messages`. Codex r5 P1 flagged this as a confused-deputy vector: a client typo or a deliberately-crafted request omitting the queue name could land on the wrong queue's purge/delete/describe path. The pre-`path.Clean` empty-segment check forecloses the bug regardless of whether the caller intended `//queues//messages` (literally — empty queue name) or `//queues/foo/messages//` (legal — trailing slash on a real queue, accepted via the §6.4 test).

After the empty-segment check, the cleaned path is split on `/`. The first segment after `queues/` is always the queue name; the rest (zero or more segments) is the sub-resource path. Sub-resource path matches: `[]` → name-only routes; `["messages"]` → messages sub-resource. Any other shape (`["messages", "x"]`, `["Messages"]`) returns 404.

**Empty queue segment rejected pre-Clean.** A request like `/admin/api/v1/sqs/queues//messages` (omitted queue name, double slash) would, after `path.Clean`, normalise to `/admin/api/v1/sqs/queues/messages` — at which point the first post-`queues/` segment is the literal string `"messages"` and the router would route the request as if the operator asked to act on a queue named `"messages"` instead of returning a validation error. Codex r5 P1 flagged that this opens a wrong-queue purge/peek hazard: a script bug that omits `${QUEUE}` interpolation could silently target whichever queue happens to be named `messages` in the cluster.

The router therefore inspects the **raw** request URL (`r.URL.EscapedPath()`, before `path.Clean`) and rejects with `400 Bad Request` + `{"code":"ValidationError","message":"empty queue name segment"}` if any consecutive `/` pair appears inside the `/queues/.../messages` segment range. The check fires before `path.Clean` so the duplicate-slash signal is still visible (Clean collapses it). The same check rejects `/admin/api/v1/sqs/queues/` (no name, trailing slash) and `/admin/api/v1/sqs/queues/messages` (legal queue name but no sub-resource — only when the operator intended messages; routed as a regular Describe / Delete of a queue literally named `messages`, which is a legitimate name and IS allowed). Test plan §6.4 adds two cases pinning the new 400 paths.
