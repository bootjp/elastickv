...
**Admin discovery seed:** The node-side Admin gRPC service is registered on every group listener (`pb.RegisterAdminServer(gs, adminServer)`, `main.go:1586-1587`) and already advertises self using `canonicalSelfAddress`, which selects the lowest-group-ID listener (`main.go:1285-1309`). PR-B must mirror that rule for peers: derive `adminMembersFromBootstrap` input from the lowest-group-ID `--raftGroupPeers` list, exclude self, and pass those node identities into `setupAdminService`. The v1 homogeneity rule guarantees the node-ID set is the same across groups, and each address in the chosen group is reachable because AdminServer is registered on every group listener. Empty `--raftGroupPeers` keeps today's `bootstrapServers` path unchanged.
