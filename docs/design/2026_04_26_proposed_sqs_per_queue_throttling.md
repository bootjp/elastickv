...
3. **Re-elected node, same queue generation, same throttle config**: `loadOrInit` finds the cached bucket and returns it as-is. The bucket keeps the token balance it had when this node last led the queue; the next charge runs the standard refill (`tokens += elapsed * refillRate`, capped at `capacity`) before deciding allow/reject. This is a correctness *feature*, not a bug — the elapsed time during which this node was a follower legitimately accrues credit, and resetting to full capacity on every re-election would let an operator trigger a burst by repeatedly bouncing leadership.
