...
- **Restart idempotency** (engine-level, `internal/raftengine/etcd/`): re-open a group dir with the same list ⇒ no re-bootstrap; re-open with a divergent flag-supplied list ⇒ PR-B's explicit configured-vs-persisted validation returns `errClusterMismatch` (add the multi-group-dir case).
