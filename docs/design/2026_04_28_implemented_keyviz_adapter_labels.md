...
Implementation note: the shipped surface uses typed labels in
`keyviz/labels.go`, a `--keyvizLabelsEnabled` startup flag, adapter
coordinator wrappers for RawKV / Redis / DynamoDB / S3 / SQS, composite
`route:<id>:<label>` bucket IDs, and explicit label copies across the
single-node JSON, fan-out JSON, and gRPC matrix paths. The default flag
value remains false, so existing rolling deploys keep legacy
`route:<id>` rows until operators opt in after every node is upgraded.
...
5. **Test coverage** — New test categories:
   - **Slot pre-creation count (PR-C+D+E, with
     `--keyvizLabelsEnabled=true`)**: register 1 route, assert
     `len(tbl.slots) == len(AllLabels) + 1`. Catches a PR-C+D+E
     ...
     `--keyvizLabelsEnabled=false`, the default): register 1
     route, assert `len(tbl.slots) == 1` (just the legacy slot);
     pins PR-B's behavior-neutral refactor goal. The `[5]Label`
     array is populated from PR-B (round-23, see §9 Q2); the
     PR-B vs PR-C+D+E counterparts differ on the flag, not on
     `AllLabels` length.
   - **`RemoveRoute` symmetric teardown (PR-C+D+E, flag on)**:
     register 1 route, `RemoveRoute(routeID)`, assert
     `len(tbl.slots) == 0` and
     `len(retiredSlots) == len(AllLabels) + 1`. Catches a
     regression where only the legacy slot is retired and the
     labeled siblings leak into the live table. **PR-B
     counterpart** (with the flag off): register 1 route,
     `RemoveRoute(routeID)`, assert `len(retiredSlots) == 1`
     — only the legacy slot was pre-created (flag off), so only
     it is retired.
