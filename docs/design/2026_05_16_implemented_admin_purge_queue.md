...
3. **Trim the route prefix** (`/admin/api/v1/sqs/queues/`) — the trailing slash is now part of the prefix so a match guarantees there is at least one further path segment — and split the remainder on `/`. The result is a slice of **raw** (still-escaped) segments. **If the very last element is empty, drop it.** This is the HTTP-mux convention for trailing-slash normalisation, and it has to happen *before* step 4's empty-segment check or `…/queues/orders/messages/` (a legitimate trailing-slash form) would split into `["orders", "messages", ""]` and be rejected by step 4 as containing an empty segment — directly contradicting the documented trailing-slash-accepted policy. Codex r14 P1 flagged this collision. Dropping only the **single trailing** empty (and never an interior empty) is the targeted fix: `/queues/orders//messages` still splits into `["orders", "", "messages"]` with the empty in the interior, and step 4 correctly rejects it; `/queues/orders/messages/` splits into `["orders", "messages", ""]`, the trailing empty is dropped, and the validation now sees `["orders", "messages"]` — both legal. (A failed trim — i.e. the path neither equals the collection root from step 2 nor starts with the prefix-with-slash — falls through to the admin router's default 404 handler; the SQS sub-router never sees it.)
4. **Validate every segment** (queue-name AND sub-resource). The rules apply uniformly to all elements of `segments`, not just `segments[0]` — Codex r13 P2 caught two cases where the earlier "queue-name-only" wording let bypass classes slip through. Reject with `400 Bad Request` + `{"code":"ValidationError","message":"invalid path segment"}` when ANY segment violates ANY of:
   - `segment == ""` — empty segment, including the **interior** case `/admin/api/v1/sqs/queues/orders//messages` (Codex r13 P2 first finding). The earlier draft only checked `segments[0]`, so `path.Clean` would normalise `orders//messages` to `orders/messages` and the malformed URL would silently succeed as a peek/purge — exactly the structural-invalid-URL-with-destructive-routing failure the routing hardening exists to prevent. Empty segments in any position are rejected at step 4.
   - `strings.ContainsRune(segment, '%')` — any percent sign in any segment is illegal. The original motivation was the `%2F` confused-deputy class on the queue-name position, but extending the same rule to sub-resource segments (Codex r13 P2 second finding) also catches **percent-encoded dot-segments** (`%2e`, `%2E`, `%2e%2e`, `%2E%2E`, etc.) that would otherwise slip past the literal `.`/`..` check below and only become traversal sequences at step 6 (decode) — by which time `path.Clean` has already run on the cleaned path. AWS queue names match `sqsQueueNamePattern` (`^[a-zA-Z0-9_-]{1,80}(\.fifo)?$`, `adapter/sqs_catalog.go:65`); the only legal sub-resource segment we recognise is the literal string `messages`. Neither needs percent-encoding for anything, so banning `%` in every segment forecloses the entire decoding-collision class — single-decode (`%2F`, `%2E`), double-decode (`%252F`, `%252E`), arbitrary-depth-nested decode (`%25252F`, …) — without iterating `url.PathUnescape` and without enumerating the encoded variants.
   - **`segment == "."` or `segment == ".."`** — dot-segment traversal in raw form. `path.Clean` collapses `..` against the preceding segment, so an input like `/admin/api/v1/sqs/queues/orders/../messages` would pass an "only-validate-segments[0]" check (`segments[0] == "orders"`, a legal name), then `path.Clean` rewrites the trimmed path to `/messages` and step 6 dispatches against a queue literally named `messages` — a wrong-queue operation reachable through a single crafted URL. Codex r7 P1 flagged this as the next confused-deputy vector after the `%`-decoding class was closed. The fix is structural: ban `.` and `..` in **every** segment at step 4, before `path.Clean` ever runs. AWS queue names cannot equal `.` or `..` anyway (the legal-character set excludes both), and the only sub-resource segment we recognise is the literal string `messages`, so the prohibition costs nothing for legitimate traffic. Percent-encoded variants (`%2e`, `%2E.`, `..%2e`, etc.) are caught by the `%`-containing rule above; the two checks together cover both the raw and the encoded dot-segment classes.
...
| `/admin/api/v1/sqs/queues/orders//messages` (empty interior)       | step 4 rejects (interior segment empty) → 400 ValidationError                 |
| `/admin/api/v1/sqs/queues/orders/%2E%2E/messages` (encoded `..`)   | step 4 rejects (segment contains `%`) → 400 ValidationError                   |
| `/admin/api/v1/sqs/queues/orders/%2e/messages` (encoded `.`)       | step 4 rejects (segment contains `%`) → 400 ValidationError                   |
...
elastickv_sqs_admin_purge_queue_total{queue, outcome}
  outcome ∈ {"ok", "forbidden", "not_leader", "not_found", "validation", "purge_in_progress", "internal_error"}

elastickv_sqs_admin_peek_queue_total{queue, outcome}
  outcome ∈ {"ok", "forbidden", "not_leader", "not_found", "validation", "throttled", "internal_error"}
```
...
Both counters carry a `validation` outcome label for the structured 400-class rejections (`ErrAdminSQSValidation` — empty / `%`-containing / dot-segment paths from §3.4 step 4, invalid cursor / oversize cursor / wrong-generation cursor from §3.1, empty / malformed queue name). Codex r14 P2 flagged that bucketing those into `internal_error` would distort alerting and make operator URL-construction mistakes indistinguishable from backend faults; the dedicated label keeps the four expected-fault classes (forbidden / not_leader / not_found / validation) cleanly separated from the unexpected-fault class (internal_error).
...
4. **Integration:** `TestSqsHandler_Messages_*` in `internal/admin/sqs_handler_test.go`:
   ...
   - path-segment validation:
     ...
     - **`GET /admin/api/v1/sqs/queues/orders/../messages`** (dot-segment escape) → 400 with the same body (pins the Codex r7 P1: `path.Clean` would rewrite to `/messages` and dispatch on a queue literally named `messages` — the step 4 `..`-segment check forecloses this regardless of what `path.Clean` would do)
     - **`GET /admin/api/v1/sqs/queues/./orders/messages`** (leading single dot) → 400 with the same body
     - **`GET /admin/api/v1/sqs/queues/orders//messages`** (empty interior segment) → 400 with the same body (Codex r13 P2: pre-r13 drafts only validated `segments[0]`, so `path.Clean` would have normalised this to `/orders/messages` and silently succeeded)
     - **`GET /admin/api/v1/sqs/queues/orders/%2E%2E/messages`** (percent-encoded `..`) → 400 with the same body (Codex r13 P2: the `%`-containing rule applies to every segment now, so encoded dot-segments are caught at step 4 instead of slipping past into step 6's decode)
     - **`GET /admin/api/v1/sqs/queues/orders/%2e/messages`** (percent-encoded `.`) → 400 with the same body
