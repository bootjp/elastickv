...
**Internal buffering model for self-test.** `out io.Writer` is write-only, so the self-test cannot read back bytes after they have already been handed to the caller. When `opts.SelfTest=true`, `EncodeSnapshot` writes the FSM into an on-disk temp file named `encode-self-test-fsm-*` under `opts.SelfTestDecodeOptions.OutRoot`, streams that file through `DecodeSnapshot`, then copies the temp file to `out` only if the self-test matched. The memory cost is O(1) beyond the existing key-sort working set. When `opts.SelfTest=false`, the FSM streams straight to `out` with no extra buffering.
...
`output_fsm_sha256` is computed AFTER `WriteTo` returns and BEFORE writing the sidecar. `MANIFEST.json` still keeps the legacy `Source.FSMCRC32C` field for compatibility, so this sidecar field is the authoritative SHA-256 integrity anchor for encoded output.
...
2. **DecodeOptions and fail-closed guards are read back from the input MANIFEST.json.** The self-test feeds `DecodeSnapshot` option flags derived from the same manifest that described `--input`; for supported fields this prevents default-vs-set mismatches. The CLI reads `MANIFEST.json` `Exclusions.{PreserveSQSVisibility,RenameS3Collisions}` and the top-level `DynamoDBLayout` field, and threads the supported decode options into the scratch `DecodeOptions`. For producer modes the encoder cannot faithfully reconstruct today, `EncodeSnapshot` fails closed before writing bytes when the corresponding adapter is enabled:
   ...
   The guards are adapter-scoped, so a Redis-only encode is not blocked by S3-only manifest flags. The decode options are still threaded for supported future-compatible paths and for disabled adapters where the unsupported data is outside the requested scope. Pinned by `TestCLIRejectsUnsupportedManifestExclusions`, `TestEncodeSnapshotRejectsUnsupportedFeatures`, `TestEncodeSnapshotUnsupportedFeaturesGatedByAdapter`, and `TestEncodeSnapshotRejectsDynamoDBJSONLLayout`.
...
| `TestEncodeSnapshotRejectsDynamoDBJSONLLayout` | A manifest-derived DynamoDB JSONL layout is rejected before the DynamoDB reverse encoder could silently skip JSONL item files. |
...
5. **Test coverage.** CLI and library tests cover the user-visible behaviors (round-trip, override validation, scope parsing, missing-manifest), the two safety nets (self-test catches corruption, ENCODE_INFO schema versioning), and review-driven robustness cases (per-`.fsm` sidecar naming, two-files-no-collision, unique scratch subdir under pinned root, cleanup on error paths, write-then-rename publish discipline, library entrypoint round-trip, unsupported manifest-derived producer modes fail closed, new `RenameS3Collisions` field round-trips, legacy manifests without the new field decode safely). Adapter-internal coverage is already pinned by M1-M5 PRs and is not re-tested here.

## Deferred decoder cleanup (parent doc §"Decoder cleanup")

The parent doc flags `internal/backup/manifest.go` `Source.FSMCRC32C` as dead for native `.fsm` output because the file has no CRC32C footer. M6 does not remove or repurpose the manifest field; it remains `fsm_crc32c` for compatibility with existing manifest readers. The CLI instead records the produced `.fsm` digest in `<output>.encode_info.json` as `output_fsm_sha256`, and that sidecar is the authoritative integrity anchor for encoded output. A manifest schema cleanup can remove or replace `Source.FSMCRC32C` in a later compatibility window.
