...
**New library entrypoint (`backup.EncodeSnapshot`).** `snapshotBuilder` / `newSnapshotBuilder` are unexported and the adapter `Encode` methods take `*snapshotBuilder`, so an external `main` package cannot directly wire them. M6 lands a high-level wrapper in `internal/backup/encode_snapshot.go` mirroring the decoder's `DecodeSnapshot`:
...
| Test | Verifies |
|---|---|
| `TestCLIRoundTripSelfTestAllAdapters` | Build the current CLI self-test fixture (SQS FIFO with dedup), run CLI with `--adapter sqs --self-test`, and assert exit 0 plus `ENCODE_INFO.json` `matched:true`. The historical test name says "AllAdapters", but the implemented fixture currently exercises SQS. |
...
5. **Test coverage.** CLI and library tests cover the user-visible behaviors that are currently pinned (SQS self-test round-trip, override validation, scope parsing, missing-manifest), the two safety nets (self-test catches corruption, ENCODE_INFO schema versioning), and review-driven robustness cases (per-`.fsm` sidecar naming, two-files-no-collision, write-then-rename publish discipline, library entrypoint round-trip, unsupported manifest-derived producer modes fail closed, sidecar symlink/hard-link refusal, mode `0600`, and legacy manifests without the new `rename_s3_collisions` field decoding safely). Adapter-internal coverage is already pinned by M1-M5 PRs and is not re-tested here.
