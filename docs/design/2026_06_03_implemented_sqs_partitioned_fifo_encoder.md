...
2. **Validate `PartitionCount`.** `readQueueMeta` rejects `partition_count` values that the live control plane cannot create: non-power-of-two values, values above the HT-FIFO cap of 32, and `partition_count > 1` on non-FIFO queues. All failures return `ErrSQSEncodeInvalidQueue` before any message is staged.
...
- `_queue.json` has a live-impossible `partition_count` shape → **`ErrSQSEncodeInvalidQueue`**. This includes non-power-of-two values, values above the HT-FIFO cap of 32, and `partition_count > 1` on non-FIFO queues. `partitionForGroup` uses the same mask contract as the live adapter, so malformed counts must fail before any message is staged. Pinned by `TestSQSEncodeRejectsNonPowerOfTwoPartitionCount`, `TestSQSEncodeRejectsTooLargePartitionCount`, `TestSQSEncodeRejectsPartitionedStandardQueue`, and `TestSQSEncodeAcceptsPowerOfTwoPartitionCount`.
