...
| `TestS3EncodeAcceptsLegitimateKeymapUserObject`                     | Bucket with a real user object named `KEYMAP.jsonl` (paired sidecar at `KEYMAP.jsonl.elastickv-meta.json`) → `isKeymapCollisionTracker` returns `tracker=false`, object round-trips as a normal S3 body, `loadBucketKeymap` is NOT called |
...
| `TestS3EncodeRejectsUnknownKeymapKind`                              | `Kind` not in `{S3LeafData, MetaCollision, SHAFallback}` → `ErrS3EncodeInvalidBucket` (forward-compat guard)                  |
