...
| 7 | Writer registry + deterministic nonce (§4.1). Implemented slices: process-start registration, storage-layer registration gate, runtime re-registration for cutover and rotation, conf-change-time pre-registration, and §5.5 `WriterRegistryForCaller` projection. `ResyncSidecar` returns the caller-specific leader registry view from `caller_full_node_id`; `GetSidecarState` reports this server's local-node projection when the registry leader view can satisfy an applied read, while followers still return the local sidecar snapshot with an empty projection. | shipped | `2026_05_26_implemented_7a_process_start_registration.md` + `2026_05_26_implemented_7a2_storage_layer_registration_enforcement.md` + `2026_05_28_implemented_7b_runtime_reregistration.md` + `2026_05_28_implemented_7b_prime_runtime_reregistration_rotation.md` + `2026_05_29_implemented_7c_confchange_time_registration.md` + `2026_07_11_implemented_7d_sidecar_registry_projection.md` |
...
| 9C-5 | §5.4 DEK retirement eligibility: the storage criteria (rewrite cursor, values-per-DEK, minRetainedTS) and the WAL-driven raft criteria (log start index, snapshot cutover), both cluster-wide with no override | shipped | — |
| 9C+ | Rotation rewrap and the rewrite job itself, admission-control wiring, remaining benchmarks (§5.2, §5.4, §6.5, §8) | open | — |
...
- **6C adds §9.1 refusal guards.** These are config / on-disk
  ...
  **6C sub-decomposition (6C-1 → 6C-4).** The §9.1 guards have
  different dependency footprints — some need only flag + sidecar
  state, others need raftengine integration, others need the
  cluster-wide membership view, others are Phase-2-specific.
  Bundling them all into one PR would either (a) block 6C-1's
  load-bearing downgrade-prevention guard on the writer-registry
  work in Stage 7 (the `local_epoch` rollback peer of
  `ErrLocalEpochExhausted`) or (b) ship a half-empty Stage 6C
  whose `ErrSidecarBehindRaftLog` and `ErrNodeIDCollision`
  paths are stubbed pending follow-ups. Splitting the milestone
  by dependency footprint keeps each sub-PR focused and lets
  6D/6E proceed once their actual blocking subset (6C-1 + 6C-2,
  with 6C-3 bundled into 6D's membership fan-out and 6C-4
  bundled into 6E's Phase-2 cutover) has shipped.

  The sub-milestones are:

    * **6C-1** — Flag + sidecar-state guards. Three sentinels:
      `ErrSidecarPresentWithoutFlag` (downgrade prevention),
      `ErrKEKRequiredWithFlag` (fail-fast on flag-on/KEK-off),
      `ErrKEKMismatch` (operator-error catch on wrong KEK for
      this data dir). Implemented as `internal/encryption/startup.go`'s
      `CheckStartupGuards`, called from `main.go run()` BEFORE
    ...
    * **6C-3** — Membership-view guards. `ErrNodeIDCollision`
      ...
      group's members; `ErrLocalEpochRollback` requires the
      writer-registry record (Stage 7). Naturally bundles with
      Stage 6D's capability fan-out which already builds that
      membership view.

    * **6C-4** — Phase-2-specific guards. `ErrEnvelopeCutoverDivergence`
      cross-checks the sidecar's `raft_envelope_cutover_index`
      against the snapshot header (which only exists once Stage 6E
      writes the cutover-index field); `ErrEncryptionNotBootstrapped`
      refuses `enable-raft-envelope` before bootstrap;
      `ErrLocalEpochOutOfRange` rejects wire-side `uint32`
      `local_epoch` values before narrowing to the §4.1 16-bit field. Bundles with
...
- **6D and 6E are the actual cutovers** (Phase 1 = storage, Phase
  ...
  can dwell in and observe indefinitely before committing to
  Phase 2 — running `encryption status` to verify
  `encryption_capable` on every replica, kicking off `rewrite`
  to expunge cleartext MVCC versions, and dwelling here as long
  as needed before issuing `enable-raft-envelope`. **Phase 1 is
  not a rollback point**, however: once `enable-storage-envelope`
  has been Raft-committed, the §9.1 sidecar-present guards
  activate cluster-wide and `encryption disable` refuses (§7.2).
  The only path back to cleartext from either Phase 1 or
  Phase 2 is dump-and-reload. The dwell exists so operators can
...
- **DEK (Data Encryption Key).** 32-byte AES key. Two DEKs are
  ...
  - **`key_id` is a 32-bit unsigned integer**, the same value that
    ...
    The leader retries the draw if it produces **`0`** (reserved
    cluster-wide as the "unset / not bootstrapped" sentinel — see
    §5.6 step 1, which checks `active.storage == 0` to decide
    whether to bootstrap) **or** if it collides with an existing
    `key_id` in the sidecar. Both retries are bounded loops in
    practice (collisions at 2⁻³² per draw, plus one explicit
    `0`-rejection); the FSM apply on every node observes the
    same final ID via the standard rotation entry. The sidecar
    uses the decimal string form of the ID as the JSON object
    key — JSON object keys must be strings, but the on-disk
    envelope and the in-memory keystore always work in the
...
- `admin.go` — gRPC server for the new `proto.EncryptionAdmin`
  ...
  **`local_epoch` wire-type rule.** The §4.1 nonce field
  ...
  Nodes that have never run with `--encryption-enabled` return
  `encryption_capable = false`, `local_epoch = 0`,
  `full_node_id = 0`; the cutover command refuses with
  `ErrCapabilityCheckFailed` in that case so the empty epoch
...
- The sidecar's `raft_applied_index` is behind the raftengine's
  persisted applied index AND the gap covers any
  encryption-relevant entry (i.e., any entry matched by §5.5's
  `isEncryptionRelevant()` predicate: `rotate-dek`,
  `rewrap-deks`, `bootstrap-encryption`,
  `enable-storage-envelope`, `enable-raft-envelope`,
  `retire-dek`). `ErrSidecarBehindRaftLog` is raised; recovered
  via `encryption resync-sidecar`. Restricting the gap check to
  `rotate-dek` only would let a missed `enable-raft-envelope`
  or `bootstrap-encryption` slip past and produce silent
  mis-dispatch / `unknown_key_id` failures on the next post-
  cutover entry — see §5.5.
