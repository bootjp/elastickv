...
// TestRunMultiplexDispatchWorkerPrioritizesHeartbeats verifies the biased-select
// invariant: heartbeat-channel messages are always drained before normal messages
// when both channels have pending entries.
func TestRunMultiplexDispatchWorkerPrioritizesHeartbeats(t *testing.T) {
	var (
		dispatched []raftpb.MessageType
		mu         sync.Mutex
	)
	e := &Engine{
		dispatchFn: func(_ context.Context, req dispatchRequest) error {
			mu.Lock()
			dispatched = append(dispatched, req.msg.Type)
			mu.Unlock()
			return nil
		},
		dispatchStopCh: make(chan struct{}),
	...
	pd := &peerQueues{
		normal:    make(chan dispatchRequest, 10),
		heartbeat: make(chan dispatchRequest, 10),
		ctx:       ctx,
		cancel:    cancel,
	}

	// Pre-fill: 3 normal messages then 3 heartbeats so both channels are
	// non-empty when the worker first runs.
	for range 3 {
		pd.normal <- prepareDispatchRequest(raftpb.Message{Type: raftpb.MsgApp})
	}
	for range 3 {
		pd.heartbeat <- prepareDispatchRequest(raftpb.Message{Type: raftpb.MsgHeartbeat})
	}

	e.dispatchWG.Add(1)
	go e.runMultiplexDispatchWorker(ctx, pd)

	// Wait until all 6 messages are dispatched, then stop.
	require.Eventually(t, func() bool {
		mu.Lock()
		defer mu.Unlock()
		return len(dispatched) == 6
	...
	mu.Lock()
	defer mu.Unlock()
	// The biased select drains heartbeat before waiting on normal, so the first
	// 3 dispatched messages must all be heartbeats.
	require.Len(t, dispatched, 6)
	for i := range 3 {
		require.Equal(t, raftpb.MsgHeartbeat, dispatched[i],
			"position %d should be MsgHeartbeat (heartbeat priority)", i)
	}
	for i := 3; i < 6; i++ {
		require.Equal(t, raftpb.MsgApp, dispatched[i],
			"position %d should be MsgApp (normal channel)", i)
	}
}
